...
def draw_detections(frame, frame_processor, detections, output_transform):
    ...
    for roi, landmarks, identity in zip(*detections):
        ...
        ### Blur only the detected face portion
        if identity.id == FaceIdentifier.UNKNOWN_ID:
            face_color = frame[ymin:ymax, xmin:xmax]
            blur = cv2.GaussianBlur(face_color, (51, 51), FrameProcessor.blur_value)
        ...
        for point in landmarks:
            ...
            ### dot on face point, like eyes, lip...
            
        textsize = cv2.getTextSize(text, cv2.FONT_HERSHEY_SIMPLEX, 0.7, 1)[0]
        cv2.rectangle(frame, (xmin, ymin), (xmin + textsize[0], ymin - textsize[1]), (0, 255, 0), cv2.FILLED)
